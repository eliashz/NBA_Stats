...
const showTeamInfo = () => {
  ...
  teamDiv.querySelector(
    '.name'
  ).textContent = `${team[0].conference} Conference`
  teamDiv.querySelector(
    '.position'
  ).textContent = `${team[0].division} Division`
...
do {
  ...
  const teamPlayers = allPlayers.data.filter(
  ...
  const showPlayerInfo = (content, tr) => {
    ...
    if (enlace) {
      //Para que el enlace solo salga en la primera columna.
  ...
  teamPlayers.map((teamPlayer) => {
    ...
    showPlayerInfo(
      inchesToCm(teamPlayer.height_feet, teamPlayer.height_inches),
      tr
    )
